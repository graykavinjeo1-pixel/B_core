...
The reproducible logical bundle is described by
`reports/core-x0/deployment_bundle_manifest.json`. It includes only the release
core artifact and the files under `crates/dockable-semantic-core/{state,config,abi}`.
It excludes `.git`, `target` caches, reports, tests, blind data, historical
runs, sandboxes, adapters, and debug binaries.
