...
def run_campaign() -> int:
    frozen = read_json(REPORT / "campaign_freeze.json")
    ...
    try:
        for epoch in range(executed + 1, budget + 1):
            ...
            expected = frozen["seed_commitments"][epoch - 1]["seed_commitment"]
            ...
            if is_success:
                atomic_json(REPORT / "final_state.json", state)
                atomic_json(
