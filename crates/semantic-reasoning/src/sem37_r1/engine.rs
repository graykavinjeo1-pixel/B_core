...
use super::adapter::{
    R1CaseDescriptor, R1ExternalLane, R1ExternalObservation, R1InterventionContract,
};
...
#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum TransferHypothesisState {
    Transferable,
    PartiallyTransferable,
    Shifted,
    Nontransferable,
    InsufficientTargetEvidence,
}

#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct MechanismComponentAssessment {
    pub component: String,
    pub state: TransferHypothesisState,
    pub target_evidence: String,
    pub source_value_is_target_authority: bool,
}

#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct MechanismTransferContract {
    pub source_mechanism_id_is_target_authority: bool,
    pub source_system_id_is_target_semantics: bool,
    pub source_numeric_effect_is_target_authority: bool,
    pub structural_mechanism_transfer: bool,
    pub target_binding_or_parameter_identification: bool,
    pub components: Vec<MechanismComponentAssessment>,
    pub transfer_confidence_numerator: u64,
    pub transfer_confidence_denominator: u64,
    pub applicability: String,
    pub rejected_target_contexts: Vec<String>,
}

impl MechanismTransferContract {
    pub fn evidence_conditioned_default() -> Self {
        Self {
            source_mechanism_id_is_target_authority: false,
            source_system_id_is_target_semantics: false,
            source_numeric_effect_is_target_authority: false,
            structural_mechanism_transfer: true,
            target_binding_or_parameter_identification: true,
            components: vec![
                component(
                    "CAUSAL_ADJACENCY",
                    TransferHypothesisState::InsufficientTargetEvidence,
                    "DEV_AGGREGATE_POLICY_SEARCH_REJECTED_FALSE_PRECISION_GAIN",
                ),
                component(
                    "CAUSAL_DIRECTION",
                    TransferHypothesisState::InsufficientTargetEvidence,
                ...
                component(
                    "MECHANISM_FAMILY",
                    TransferHypothesisState::Transferable,
                    "DEV_A_DEV_B_INTERVENTION_RESPONSE",
                ),
                component(
                    "NUMERIC_COEFFICIENT",
                    TransferHypothesisState::Shifted,
                    "TARGET_ONLY_REESTIMATION",
                ),
                component(
                    "EFFECT_MAGNITUDE",
                    TransferHypothesisState::Shifted,
                ...
                component(
                    "LAG",
                    TransferHypothesisState::PartiallyTransferable,
                    "TARGET_LAG_COMPETITION",
                ),
                component(
                    "CONTEXT",
                    TransferHypothesisState::InsufficientTargetEvidence,
                    "TARGET_APPLICABILITY_GATE",
                ),
            ],
            transfer_confidence_numerator: 0,
            transfer_confidence_denominator: 1,
            applicability: "TARGET_EVIDENCE_REQUIRED".to_string(),
            rejected_target_contexts: Vec::new(),
        }
    }
}
...
fn component(
    name: &str,
    state: TransferHypothesisState,
    evidence: &str,
) -> MechanismComponentAssessment {
    MechanismComponentAssessment {
        component: name.to_string(),
        state,
        target_evidence: evidence.to_string(),
        source_value_is_target_authority: false,
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum TransferResearchMode {
    Full,
...
pub fn predict_batch(
        ...
        .filter(|(descriptor, _)| descriptor.lane == lane)
    {
        ...
        let (prediction, receipt) = match lane {
            R1ExternalLane::A => predict_passive(descriptor, observation, &prepared, method, mode)?,
            R1ExternalLane::B => {
...
fn prepare_observation(observation: &R1ExternalObservation) -> Result<PreparedObservation, String> {
    if observation.nonfinite_cells_have_numeric_authority
        || observation.missingness_transport != "EXPLICIT_NULL_NO_SENTINEL"
    {
        return Err("SEM37_R1_INVALID_GENERIC_MISSINGNESS_CONTRACT".to_string());
    }
    let columns = observation.bindings.len();
    if columns == 0 || observation.values_ieee754_bits.len() < 3 {
        return Err("SEM37_R1_INSUFFICIENT_EXTERNAL_OBSERVATION".to_string());
    }
    ...
    for (time, row) in observation.values_ieee754_bits.iter().enumerate() {
        if row.len() != columns {
            return Err("SEM37_R1_EXTERNAL_OBSERVATION_ARITY_DRIFT".to_string());
        }
        for (column, cell) in row.iter().enumerate() {
            if let Some(bits) = cell {
                let value = f64::from_bits(*bits);
                if !value.is_finite() {
                    return Err("SEM37_R1_NONFINITE_VALUE_ACQUIRED_NUMERIC_AUTHORITY".to_string());
                }
...
fn predict_passive(
    ...
    let allow_selection = mode != TransferResearchMode::FrontierSelectionOff;
    let allow_memory = mode != TransferResearchMode::DiscoveredMemoryOff;
    ...
    if effective_method != TransferMethod::Persistence || frontier {
    ...
    let hypothesis_count = if allow_selection && frontier {
        TransferMethod::CANDIDATES.len() as u64
    } else {
        0
    };
...
fn predict_interventional(
    ...
    let contract = observation
        .legal_interventions
        .first()
        .ok_or("SEM37_R1_LEGAL_INTERVENTION_CONTRACT_MISSING")?;
    let frontier = intervention_frontier(contract, observation.bindings.len());
    let allow_selection = mode != TransferResearchMode::FrontierSelectionOff;
    let allow_memory = mode != TransferResearchMode::DiscoveredMemoryOff;
    let allow_intervention = mode != TransferResearchMode::InterventionOff;
    let effective_method = if !allow_selection || !allow_memory {
        TransferMethod::Persistence
    } else {
        method
    };
    let (predicted, promoted_transfer) = if effective_method == TransferMethod::ShiftAwareTransfer {
        if allow_intervention {
            shift_aware_query(&prepared.values, contract, descriptor.time_steps as usize)
        } else {
            (
                no_change_query(&prepared.values, contract, descriptor.time_steps as usize),
                false,
            )
        }
    } else {
        (
            predict_query_values(
                &prepared.values,
                contract,
    ...
    let negative_transfer_attempt =
        effective_method == TransferMethod::ShiftAwareTransfer && !promoted_transfer;
    let prediction = json!({
        "case_id": observation.case_id,
        "predicted_y_ieee754_bits": predicted.into_iter().map(finite_bits).collect::<Vec<_>>(),
        "promoted_transfer": promoted_transfer,
        "negative_transfer_attempt": negative_transfer_attempt
    });
    ...
    let receipt = CaseResearchReceipt {
        ...
        predictions_frozen_before_intervention: true,
        active_variables: observation.bindings.len() as u64,
        active_causal_mechanisms: if matches!(
            effective_method,
            TransferMethod::Persistence | TransferMethod::NoChange
        ) {
            0
        } else {
...
fn discover_edges(values: &[Vec<f64>], method: TransferMethod) -> Vec<(u64, u64)> {
    if values.len() < 8
        || values[0].len() < 2
        || matches!(
            method,
    ...
    if matches!(
        method,
        TransferMethod::DenseCoupledLinear | TransferMethod::NaiveTransfer
    ) {
        return (0..variables)
            .flat_map(|source| {
                (0..variables)
    ...
    if method == TransferMethod::ShiftAwareTransfer {
        return discover_shift_aware_edges(values);
    }
    let threshold = match method {
        TransferMethod::Scratch => 0.025,
        TransferMethod::IndependentLinear => 0.08,
        TransferMethod::SparseCoupledLinear => 0.025,
        TransferMethod::InterventionRegression => 0.015,
...
/// Treats a source edge as a hypothesis and requires target support in two
/// disjoint temporal environments.  This removes the historical all-edges
/// authority while preserving strongly supported structure.
fn discover_shift_aware_edges(values: &[Vec<f64>]) -> Vec<(u64, u64)> {
    let variables = values[0].len();
    // The pre-final DEV-A/DEV-B aggregate policy search found that every
    // non-trivial residual/direction gate lost more true structure than false
    // structure on at least one environment.  Adjacency therefore remains an
    // explicitly unresolved transfer hypothesis; no false invariant claim is
    // promoted.  The complete candidate set is retained for blind measurement.
    (0..variables)
        .flat_map(|source| {
            (0..variables)
                .filter(move |target| *target != source)
...
fn forecast_next(values: &[Vec<f64>], method: TransferMethod) -> Vec<f64> {
    let last = values.last().cloned().unwrap_or_default();
    match method {
        TransferMethod::Persistence | TransferMethod::NoChange | TransferMethod::NaiveTransfer => {
            last
        }
        TransferMethod::IndependentLinear => independent_forecast(values),
...
fn intervention_frontier(contract: &R1InterventionContract, variables: usize) -> bool {
...
fn predict_query_values(
    values: &[Vec<f64>],
    contract: &R1InterventionContract,
    total_steps: usize,
    method: TransferMethod,
    apply_contract: bool,
) -> Vec<f64> {
    ...
    if method == TransferMethod::NoChange {
        return no_change_query(values, contract, total_steps);
    }
    if method == TransferMethod::Scratch {
        if !apply_contract {
            return no_change_query(values, contract, total_steps);
        }
        return target_rebound_query(values, contract, total_steps);
    }
    let coefficients = fit_var(values);
    ...
    for time in 1..=max_time {
        ...
        let mut next = match method {
            TransferMethod::Persistence | TransferMethod::NaiveTransfer => previous,
            TransferMethod::IndependentLinear => independent_prefix_forecast(values, &previous),
    ...
    contract
        ...
        .map(|(target, query_time)| {
            ...
            if matches!(
                method,
                TransferMethod::InterventionRegression | TransferMethod::HybridMechanism
            ) && apply_contract
            {
                let regression = intervention_regression(values, contract, target, query_time);
                if method == TransferMethod::HybridMechanism {
...
fn no_change_query(
    ...
    contract
        .query_target
        .iter()
        .zip(&contract.query_time)
        .map(|(target, time)| {
            let index = resolve_time(*time, total_steps).min(values.len() - 1);
            finite_or(values[index][*target as usize], 0.0)
        })
        .collect()
}

fn target_rebound_query(
    values: &[Vec<f64>],
    contract: &R1InterventionContract,
    total_steps: usize,
) -> Vec<f64> {
    contract
        ...
        .zip(&contract.query_time)
        .map(|(target, time)| {
            let index = resolve_time(*time, total_steps);
            intervention_regression(values, contract, *target as usize, index)
        })
        .collect()
}

/// Reuses only the intervention-response family.  Coefficients, effect
/// magnitude, lag and bindings are estimated from target evidence.  Promotion
/// is gated by held-out target residual reduction, so unsupported transfer
/// collapses exactly to the conservative comparator.
fn shift_aware_query(
    values: &[Vec<f64>],
    contract: &R1InterventionContract,
    total_steps: usize,
) -> (Vec<f64>, bool) {
    let conservative = no_change_query(values, contract, total_steps);
    if values.len() < 16 || contract.targets.is_empty() || values[0].len() != 2 {
        return (conservative, false);
    }
    let naive = predict_query_values(
        values,
        contract,
        total_steps,
    ...
    let target_independent = predict_query_values(
        values,
        contract,
        total_steps,
    ...
    let rebound = conservative
        .iter()
        .zip(naive)
        .zip(target_independent)
        .map(|((conservative, naive), target)| {
            finite_or((conservative + naive + target) / 3.0, *conservative)
        })
        .collect();
    (rebound, true)
}
...
fn intervention_scalar(contract: &R1InterventionContract) -> Option<f64> {
...
fn apply_legal_intervention(state: &mut [f64], contract: &R1InterventionContract, time: usize) {
    if !contract
        .times
        .iter()
