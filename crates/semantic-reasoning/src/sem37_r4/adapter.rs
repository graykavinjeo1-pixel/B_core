...
impl R4ExternalSet {
    const fn evaluator_name(self) -> &'static str {
        match self {
            Self::DevF => "R4_DEV_F",
            Self::FinalG => "R4_FINAL_G",
        }
    }

    const fn compatibility_set(self) -> R1ExternalSet {
        match self {
            Self::DevF => R1ExternalSet::R1DevA,
            Self::FinalG => R1ExternalSet::R1FinalC,
        }
    }
}

#[derive(Debug, Clone)]
pub struct R4ExternalEvaluatorClient {
    python: PathBuf,
    script: PathBuf,
}

impl R4ExternalEvaluatorClient {
    pub fn from_vault(vault: &Path) -> Result<Self, String> {
        ...
        let script = vault.join("sem37_r4_external_evaluator.py");
        if !python.is_file() || !script.is_file() {
            return Err("SEM37_R4_EXTERNAL_EVALUATOR_RUNTIME_MISSING".to_string());
        }
    ...
    pub fn request(&self, payload: &Value) -> Result<Value, String> {
        let mut child = Command::new(&self.python)
            ...
            .stderr(Stdio::piped())
            .spawn()
            .map_err(|error| format!("SEM37_R4_SPAWN_EVALUATOR:{error}"))?;
        child
            .stdin
            .take()
            .ok_or("SEM37_R4_EVALUATOR_STDIN_MISSING")?
            .write_all(&serde_json::to_vec(payload).map_err(|error| error.to_string())?)
            .map_err(|error| format!("SEM37_R4_WRITE_EVALUATOR:{error}"))?;
        let output = child
            .wait_with_output()
            .map_err(|error| format!("SEM37_R4_WAIT_EVALUATOR:{error}"))?;
        let response: Value = serde_json::from_slice(&output.stdout).map_err(|error| {
            format!(
                "SEM37_R4_PARSE_EVALUATOR:{error}:{}",
                String::from_utf8_lossy(&output.stderr)
            )
        })?;
        if !output.status.success() || response["status"].as_str() != Some("PASS") {
            return Err(format!(
                "SEM37_R4_EVALUATOR_REJECTED:{}",
    ...
    pub fn catalog(&self, set: R4ExternalSet) -> Result<R1ExternalCatalog, String> {
        let mut value = self.request(&json!({"action": "catalog", "set": set.evaluator_name()}))?;
        value["set"] =
        ...
        serde_json::from_value(value).map_err(|error| format!("SEM37_R4_CATALOG_SCHEMA:{error}"))
    }
    ...
    pub fn observe(
        ...
        serde_json::from_value(value)
            .map_err(|error| format!("SEM37_R4_OBSERVATION_SCHEMA:{error}"))
    }
