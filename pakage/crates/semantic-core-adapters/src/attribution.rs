...
impl AttributionAnalyzer {
    pub fn analyze(&self, text: &str, frames: &[PredicateFrameIR]) -> AttributionGraphIR {
        ...
        let mut pending = mentions
            ...
            .collect::<Vec<_>>();
        resolve_coordinated_attribution_ellipsis(&mut pending);
        if pending.is_empty() {
...
fn pending_from_mention(
    ...
    let punctuation_clause_start = text[..mention.start]
        .rfind(['.', '?', '!', ';', '\n', '\r'])
        .map_or(0, |position| position + 1);
    let clause_start = index
        .checked_sub(1)
        .and_then(|prior| mentions.get(prior))
        .filter(|prior| contains_hangul(text) && korean_report_coordinator(&prior.form))
        .map_or(punctuation_clause_start, |prior| prior.end);
    ...
    let (actor_surface, actor_kind, actor_end) = if korean {
        extract_korean_actor(
            ...
            clause_start,
            mention.start,
            outermost || korean_report_coordinator(&mention.form),
        )
    } else {
...
fn extract_english_actor(
    ...
    let segment = [", but ", "; but ", ", however ", "; however "]
        ...
        .max()
        .map_or(segment, |position| &segment[position..]);
    let segment = strip_english_discourse_repair_prefixes(segment)
...
fn korean_report_coordinator(form: &str) -> bool {
    matches!(form, "했는데" | "말했지만" | "말했으나")
}

fn resolve_coordinated_attribution_ellipsis(pending: &mut [PendingAttribution]) {
    let mut prior_subject = None::<String>;
    for item in pending {
        let surface = item.proposition.surface_text.trim();
        if let Some(subject) = explicit_proposition_subject(surface) {
            if let Some(prior) = prior_subject.as_deref() {
                let abbreviated = subject.split_whitespace().count() == 1
                    && prior
                        .split_whitespace()
                        .any(|token| token.eq_ignore_ascii_case(&subject));
                if abbreviated {
                    let predicate = surface[subject.len()..].trim();
                    item.proposition.surface_text = format!("{prior} {predicate}");
                    item.proposition.normalized_text =
                        normalize_proposition(&item.proposition.surface_text);
                    item.proposition.polarity =
                        proposition_polarity(&item.proposition.surface_text);
                    continue;
                }
            }
            prior_subject = Some(subject);
            continue;
        }
        let Some(subject) = prior_subject.as_deref() else {
            continue;
        };
        let predicate = surface
            .strip_prefix("it ")
            .or_else(|| surface.strip_prefix("that "))
            .unwrap_or(surface)
            .trim();
        if predicate.is_empty() {
            continue;
        }
        item.proposition.surface_text = format!("{subject} {predicate}");
        item.proposition.normalized_text = normalize_proposition(&item.proposition.surface_text);
        item.proposition.polarity = proposition_polarity(&item.proposition.surface_text);
    }
}

fn explicit_proposition_subject(surface: &str) -> Option<String> {
    if contains_hangul(surface) {
        let tokens = surface.split_whitespace().collect::<Vec<_>>();
        let subject_end = tokens.iter().rposition(|token| {
            ["이", "가", "은", "는"]
                .iter()
                .any(|particle| token.ends_with(particle))
        })?;
        return Some(tokens[..=subject_end].join(" "));
    }
    let tokens = surface.split_whitespace().collect::<Vec<_>>();
    let predicate_start = tokens.iter().position(|token| {
        matches!(
    ...
    let subject = tokens[..predicate_start].join(" ");
    (!matches!(subject.as_str(), "it" | "that" | "this")).then_some(subject)
}

fn strip_english_discourse_repair_prefixes(mut segment: &str) -> &str {
    const PREFIXES: &[&str] = &[
        "correction:",
        "correction,",
        "to correct that,",
...
fn extract_korean_actor(
    ...
    let local = raw_segment.find(selected.0)?;
...
mod tests {
    ...
    #[test]
    fn discourse_repair_prefix_is_not_part_of_the_attributed_actor() {
        let graph = analyze("Correction: Nora reports that the build succeeded.");
        assert!(graph.validate());
    ...
    #[test]
    fn korean_repair_prefix_is_not_mistaken_for_the_attributed_actor() {
        let graph = analyze("사실은 하람은 캐시가 정상이라고 믿었다.");
        assert!(graph.validate());
        assert_eq!(graph.actors.len(), 1);
        assert_eq!(graph.actors[0].normalized_label, "하람");
        assert_eq!(graph.propositions[0].normalized_text, "캐시가 정상");
    }

    #[test]
    fn coordinated_english_reports_keep_each_actor_and_resolve_pronominal_theme() {
        let graph = analyze("Mina says the Alder cache succeeded, but Jisoo says it failed.");
        assert!(graph.validate());
        assert_eq!(graph.attributions.len(), 2);
        assert_eq!(graph.actors[0].normalized_label, "mina");
        assert_eq!(graph.actors[1].normalized_label, "jisoo");
        assert_eq!(
            graph.propositions[0].normalized_text,
            "the alder cache succeeded"
        );
        assert_eq!(
            graph.propositions[1].normalized_text,
            "the alder cache failed"
        );
    }

    #[test]
    fn coordinated_korean_reports_keep_each_actor_and_resolve_omitted_theme() {
        let graph = analyze("민수는 Aster 캐시가 성공했다고 했는데 지수는 실패했다고 했어.");
        assert!(graph.validate(), "{graph:#?}");
        assert_eq!(graph.attributions.len(), 2, "{graph:#?}");
        assert_eq!(graph.actors[0].normalized_label, "민수");
        assert_eq!(graph.actors[1].normalized_label, "지수");
        assert!(graph.propositions[0]
            .normalized_text
            .contains("aster 캐시가"));
        assert!(graph.propositions[1]
            .normalized_text
            .contains("aster 캐시가"));
        assert!(graph.propositions[1].normalized_text.contains("실패"));
    }
}
